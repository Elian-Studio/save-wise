...
export function SchemeDetail() {
  ...
  return (
    <div className="min-h-screen bg-pp-bg pb-24">
      ...
      <section
        className="px-5 pt-12 pb-11 text-white sm:px-8"
      ...
      <div className="mx-auto max-w-[880px] px-5 pt-9 sm:px-8">
        {/* 이런 사람 칩 */}
        <div className="flex flex-wrap gap-2">
          {scheme.fit.map((f) => (
            <span
              key={f}
              className="rounded-full border border-pp-line bg-pp-card px-4 py-2.5 text-sm font-bold text-pp-body"
            >
              {f}
            </span>
          ))}
        ...
        >
          {scheme.benefits.map((b) => (
            <div key={b.title} className="rounded-[18px] border border-pp-line bg-pp-card p-5.5">
              <div className="text-base font-extrabold text-pp-ink">{b.title}</div>
              <div className="mt-1.5 text-[14.5px] font-medium leading-[1.55] text-pp-muted">
                {b.desc}
              </div>
            </div>
          ))}
        ...
        {/* 발급 카드/방식 픽 */}
        <div className="mt-9">
          <div className="mb-1 text-[17px] font-extrabold text-pp-ink">{scheme.pick.title}</div>
          <div className="mb-4 text-[14.5px] font-medium leading-[1.5] text-pp-muted">
            {scheme.pick.hint}
          </div>
          <div className="flex flex-col gap-2.5">
            {scheme.picks.map((raw, i) => {
              ...
              return (
                <div
                  key={`${p.name}-${i}`}
                  className="flex items-start gap-4 rounded-[18px] border border-pp-line bg-pp-card p-5"
                >
                  <div className="flex-1">
                    <div className="flex flex-wrap items-center gap-2.5">
                      <span className="text-[16.5px] font-extrabold text-pp-ink">{p.name}</span>
                      <span
                    ...
                    <div className="mt-2 text-[14.5px] font-medium leading-[1.55] text-pp-muted">
                      {p.desc}
                    </div>
                    {p.applyUrl && (
                      <a
        ...
        {/* 발급 스텝 */}
        <div className="mt-9">
          <div className="mb-3.5 text-[17px] font-extrabold text-pp-ink">발급은 이렇게</div>
          <div className="flex flex-col">
            {scheme.steps.map((step, i) => (
              <div
                key={step}
                className="flex items-start gap-3.5 border-b border-pp-line py-3.5"
              >
                <span
                ...
                <span className="pt-0.5 text-[15px] font-semibold leading-[1.5] text-pp-body">
                  {step}
                </span>
              </div>
            ))}
