...
export function HomeGuide({ onStartQuiz }: { onStartQuiz: () => void }) {
  return (
    <section className="mx-auto mt-24 w-full max-w-[880px] text-left">
      {/* 1. 5종 한눈에 */}
      <h2 className="text-[clamp(24px,3.5vw,32px)] font-extrabold tracking-[-0.03em] text-pp-ink">
        2026 교통카드 5종, 한눈에
      </h2>
      <div className="mt-4 flex flex-col gap-3.5">
        <p className="text-[15.5px] font-medium leading-[1.7] text-pp-body">
      ...
      <div className="mt-6 overflow-x-auto rounded-[18px] border border-pp-line">
        <table className="min-w-[640px] w-full border-collapse text-left text-[13.5px]">
          <thead>
            <tr className="border-b border-pp-line bg-pp-card">
              <th scope="col" className="px-4 py-3 font-bold text-pp-muted">
                구분
              </th>
              {SCHEMES.map((s) => (
                <th key={s.id} scope="col" className="px-4 py-3 font-extrabold text-pp-ink">
          ...
          <tbody>
            {COMPARE_KEYS.map((key, i) => (
              <tr key={key} className={i < COMPARE_KEYS.length - 1 ? 'border-b border-pp-line' : ''}>
                <th scope="row" className="px-4 py-3 font-bold text-pp-muted whitespace-nowrap">
                  {key}
                </th>
                {SCHEMES.map((s) => (
                  <td key={s.id} className="px-4 py-3 font-medium leading-[1.5] text-pp-body">
                    {s.compareRows[i].v}
                  </td>
                ))}
              </tr>
      ...
      {/* 2. 결정 가이드 */}
      <h2 className="mt-16 text-[clamp(24px,3.5vw,32px)] font-extrabold tracking-[-0.03em] text-pp-ink">
        어떤 카드를 골라야 하나
      </h2>
      <div className="mt-4 flex flex-col gap-3.5">
        <p className="text-[15.5px] font-medium leading-[1.7] text-pp-body">
      ...
      <div className="mt-4 flex flex-col gap-3">
        {HOME_FAQ.map((f) => (
          <div key={f.q} className="rounded-[18px] border border-pp-line bg-pp-card p-5">
            ...
            <div className="mt-1.5 text-[14.5px] font-medium leading-[1.6] text-pp-muted">{f.a}</div>
          </div>
        ))}
      </div>
    </section>
