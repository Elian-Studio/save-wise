...
for (const seo of ROUTE_SEO) {
  ...
  // head 주입은 lastIndexOf 기반 — 주석 등에 '</head>' 리터럴이 있어도 항상 "진짜" 닫는 태그(마지막
  // 일치) 앞에 삽입된다. 첫 일치 replace가 주석 안을 잡아 CSS/JS가 주석에 갇힌 사고(2026-07-06)의 재발 방지.
  const headClose = template.lastIndexOf('</head>');
  const withHead = template.slice(0, headClose) + `    ${renderHead(seo)}\n  ` + template.slice(headClose);
  // 함수 치환 — 삽입 문자열의 $& 등 특수 replacement 패턴이 잘못 해석되는 사고 방지.
  const html = withHead.replace(rootMarker, () => `<div id="root">${body}</div>`);
