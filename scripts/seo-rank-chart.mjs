...
var curr = snaps[snaps.length - 1];
document.getElementById('tableTitle').textContent = '전체 키워드 변화 — ' +
  (prev ? prev.date + ' → ' + curr.date : curr.date + ' (베이스라인, 비교 대상 없음)');

function chip(pv, cv) {
  var fmt = function (v) { return v === undefined ? '·' : (v === null ? '미노출' : v + '위'); };
  if (cv === undefined && pv === undefined) return '<span class="chip na">·</span>';
  if (!prev) return '<span class="chip flat">' + fmt(cv) + '</span>';
  var pn = typeof pv === 'number', cn = typeof cv === 'number';
  if (!pn && !cn) return '<span class="chip na">미노출</span>';
  var base = fmt(pv) + ' → ' + fmt(cv) + ' ';
  if (pn && cn) {
    var d = pv - cv; // 순위 숫자가 줄면 상승
    if (d > 0) return base + '<span class="chip up">▲' + d + '</span>';
    if (d < 0) return base + '<span class="chip down">▼' + (-d) + '</span>';
    return base + '<span class="chip flat">—</span>';
  }
  return base + (cn ? '<span class="chip new">NEW</span>' : '<span class="chip out">OUT</span>');
}

var rows = kws.map(function (kw) {
  return '<tr><td class="kw">' + kw + '</td>' +
    '<td>' + chip(prev ? rankOf(prev, kw, 'naver') : undefined, rankOf(curr, kw, 'naver')) + '</td>' +
    '<td>' + chip(prev ? rankOf(prev, kw, 'google') : undefined, rankOf(curr, kw, 'google')) + '</td></tr>';
});
