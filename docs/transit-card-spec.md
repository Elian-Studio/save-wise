...
## 3. 대안 — 기후동행카드 (서울 정액) — **충전 종료(2026-07)**
> **종료 공지**(서울시 교통실 보도, 2026-07-03, https://news.seoul.go.kr/traffic/archives/517575):
> - 선불 30일권 충전 종료: **2026-07-31**
> - 후불 기후동행카드 충전 종료: **2026-08-31**
> - 이미 충전한 카드는 만료일까지 사용 가능(유효기간 내 무제한 유지).
> - 청년 월 7,000원 환급(페이백) 포함 페이백은 **6월 충전분까지** 지급.
> - 서울시는 K-패스로의 전환을 안내.
>
> **제품 결정**: 기후동행카드는 **추천 엔진(`recommend`)에서 제외**한다(`transitSchemeRec.ts` climate 블록 삭제). SEO·검색 유입(종료 관련 질의 포함) 유지를 위해 상세 페이지(`/transit/cards/climate`)는 남기되, `Scheme.endNotice` 배너로 종료 사실을 고지하고 비교표·홈 카피·FAQ를 종료 기준으로 현행화한다. climate 전용 입력이던 `bike`(따릉이) 퀴즈 문항·`QuizAnswers.bike` 필드도 함께 제거.

- 이용가능(종료 전 기준): `region==='seoul' && transit!=='wide'` (서울 외 하차·GTX·신분당 제외).
